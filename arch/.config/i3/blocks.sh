...
_DISK_ () {
    ...
    disk_usage0=$(df -h / | awk '/\/dev\//{print $3-G"/"$2}');
    disk_usage1=$(df -h /home | awk '/\/dev\//{print $3-G"/"$2}');
    disk_usage2=$(df -h /share | awk '/\/dev\//{print $3-G"/"$2}');
    echo " $disk_usage0  $disk_usage1  $disk_usage2"
}
...
_VOLUME_ () {
    Vol=$(amixer -c 1 get Master | awk '/Mono:/{print $4}' | tr -d "[ %]")
    Mute=$(amixer -c 1 get Master | awk '/Mono:/{print $6}' | tr -d "[-]")
